...
function HttpSendAlert(message) 
	print("message vlosite: "..message)
	b,c,h = fa.request{url = "http://192.168.2.101/process.php?deviceID=310&callerID=310&messagetypeID=MESS_TYPE_SCHEME&schemeID=289&commandvalue="..message }
	return c
end
    ...
    if attr.mode == "file" then
		if file > lastuploaded then
			...
			if httpmotion==false then
				print(HttpSendAlert(folder.."/"..file))
				httpmotion = true
			end
